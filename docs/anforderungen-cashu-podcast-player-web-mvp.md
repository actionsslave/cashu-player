...
| FR-09 | Die Abo-Liste zeigt Cover, Titel und die **Gesamtzahl der Episoden im Feed** — nicht die Anzahl der lokal gespeicherten und nicht die der angezeigten. Sie beantwortet die Frage, wie umfangreich der Podcast ist, und wird bei jedem Refresh nachgezogen. Trägt ein Abo den Wert noch nicht, weil es vor dieser Änderung angelegt wurde, zeigt die Liste ersatzweise den gespeicherten Bestand. Abbestellen erfolgt nach Bestätigungsdialog und löscht auch die Episodendaten. Präzisiert am 01.09.2026. |
| FR-10 | Pro Feed werden die 50 neuesten Episoden geladen und gespeichert. Die Bibliothek zeigt sie seit dem Podcast-Nav-Handoff (03.09.2026) **vollständig**: in der Episoden-Ansicht als flache Liste über alle Abos, auf der Show-Seite je Podcast, jeweils absteigend nach Datum. Je Zeile stehen Show beziehungsweise Datum als Kicker, der Titel, eine einzeilige Beschreibung, die Restzeit und ein Abspielknopf. Der Titel ist anklickbar und wählt die Episode zur Wiedergabe aus. Veröffentlichungsdatum, Dauer und Beschreibung werden weiterhin geparst und gespeichert, aber nicht in der Liste dargestellt: Die Beschreibungen der Feeds sind lang und enthalten rohes HTML, was die Liste unbrauchbar macht. Abruf- und Anzeigemenge sind getrennt konfigurierbar (`EPISODES_PER_FEED`, `EPISODES_VISIBLE`), damit eine geänderte Anzeigemenge kein erneutes Laden aller Feeds erzwingt. Geändert am 01.09.2026. |
...
| FR-22 | Zur aufgelösten Identität lädt die App das `kind:10019`-Event (Mints, Relays, P2PK-Pubkey), cached es für 24 h und prüft, ob mindestens ein dort gelisteter Mint in der eigenen erlaubten Mint-Liste enthalten ist **und die Einheit Sat führt**. NIP-61 erlaubt an jedem `mint`-Tag Marker mit den unterstützten Basiseinheiten; ein Mint, der nur andere Einheiten nennt, scheidet aus. Fehlen die Marker, gilt keine Einschränkung. Ergänzt am 02.09.2026. |
...
| FR-27 | Ein Nutzap wird als `kind:9321`-Event gebaut: Proofs P2PK-gelockt auf den Pubkey aus `kind:10019` mit vorangestelltem `02` — **nur bei Mints, die NUT-11 melden**, sonst bricht die Zahlung vor dem Swap ab, weil ein Nutzap ohne durchgesetztes P2PK für jeden ausgebbar wäre (ergänzt 02.09.2026), Tags `proof`, `unit`, `u` (Mint-URL exakt wie im `kind:10019`) und `p` (Empfänger); signiert über die Extension und an die Relays aus `kind:10019` publiziert. |
| FR-28 | Boost: Der Nutzer wählt einen Betrag aus vier Vorgaben (210, 2 100, 4 200, 21 000 Sat — Vielfache von 21, so im Design-Handoff festgelegt und dort ausdrücklich als gewollt bezeichnet; geändert am 02.09.2026) oder gibt einen freien Betrag ein, optional mit Nachricht bis 280 Zeichen; die Nachricht wird zum `content` des `kind:9321`, davor stehen Folgentitel und Hörposition im Format `hh:mm:ss` (`<Folge> · <hh:mm:ss> — <Nachricht>`). Der Podcast selbst steht nicht im Text — er ist der Empfänger. Derselbe Kontext geht zusätzlich als Tags mit (OQ-02). Maßgeblich ist die Position **zum Zeitpunkt, an dem der Dialog geöffnet wird** — sie wird dort eingefroren. Sonst trüge der Boost eine andere Stelle als die, die der Nutzer beim Klick gesehen hat, und die Vorschau im Dialog liefe während des Tippens mit. Präzisiert am 02.09.2026. |
...
| OQ-02 | Wie werden Podcast-, Episoden- und Zeitkontext maschinenlesbar mitgesendet, wenn es kein TLV gibt? | **Entschieden am 02.09.2026, beides.** Am `kind:9321` stehen jetzt Tags mit den Feldnamen aus blip-0010: `podcast`, `episode`, `podcast_guid`, `episode_guid`, `ts`. Das ist ein Vorschlag, keine Konvention — bisher liest sie niemand. Deshalb steht derselbe Kontext zusätzlich lesbar im `content`, denn Wallets zeigen heute nur den: Ein bei Nodesignal angekommener Boost wurde ohne Folgenzuordnung verbucht, genau daran. Der Anlass war die Empfängerseite, auf die diese Frage gewartet hat. |
...
| OQ-09 | Bleibt die erlaubte Mint-Liste eine Build-Konstante, oder soll die App später jeden Mint annehmen, den ein Podcast in seinem `kind:10019` nennt? | Für den MVP: Build-Konstante, Schnittmenge nach FR-22, sonst Zahlungen aus (FR-23). Die Liste ist **keine Bequemlichkeit**: Ein Mint ist ein Verwahrer, und nach dem Swap bleibt das Wechselgeld bei *ihm* liegen (`addProofs(mintUrl, locked.keep)` in `pay.ts`). Ein Podcast, der einen selbst betriebenen Mint nennt, bekäme damit nicht nur die Zahlung, sondern die Obhut über den Rest. Dazu kommen CORS (A-02) und NUT-11 als Funktionsbedingung. Vorschlag für die Ausbaustufe: Vertrauen zur **Laufzeit** erteilen statt im Build — unbekannter Mint wird live geprüft (NUT-11/12, Fees, Erreichbarkeit), dem Nutzer vorgelegt und mit einem **Guthaben-Limit** versehen. Die saubere allgemeine Lösung wäre Melt-und-Remint über Lightning; sie widerspricht aber der Prämisse aus Kapitel 1, dass Ecash gerade wegen der Lightning-Kosten bei Kleinstbeträgen gewählt wurde. Aufgenommen am 02.09.2026. |
| OQ-10 | Was passiert, wenn die Schnittmenge nicht leer ist, das Guthaben aber beim *falschen* der erlaubten Mints liegt? | Heute: Die Zahlung scheitert. `sendNutzap` geht `target.mints` durch und ruft `wallet.reserve(amount, candidate)`; es mintet nichts nach und verschiebt nichts. Reicht das Guthaben bei keinem der gemeinsamen Mints, endet der Versuch mit `InsufficientFundsError`, obwohl die Wallet insgesamt genug hält. Mit einem einzigen Mint fällt das nicht auf; mit mehreren wird es der nächste Stolperstein. Zu entscheiden: Guthaben bewusst auf einen Mint bündeln, dem Nutzer den Ziel-Mint sichtbar machen, oder Guthaben bei Bedarf zwischen Mints verschieben — Letzteres wieder nur über Lightning und damit zu den Kosten aus OQ-09. Aufgenommen am 02.09.2026. |
